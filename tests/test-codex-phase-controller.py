...
class PhaseControllerTests(unittest.TestCase):
    ...
    def test_happy_pipeline_emits_native_actions_and_finishes_without_finalize(self) -> None:
        ...
        self.assertIn("# Project learning", action.prompt)
        self.assertIn(".codex/skills/<kebab-case-name>/SKILL.md", action.prompt)
        self.assertIn("Make at most", action.prompt)
        self.assertIn("two repair attempts", action.prompt)
        self.assertIn("Never apply a global change automatically", action.prompt)
        self.assertIn(str(controller.store.paths.progress), action.prompt)
        action = controller.record_result(action.action_id, self.result("clean"))
