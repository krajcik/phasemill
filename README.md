...
For local development, replace `krajcik/phasemill --ref v1.7.0` with the
...
native read-only review, required Pi review, finalization, and project
...
publish, deploy, worktree cleanup, and user-global learning application remain
separate workflows.
...
## Self-improving project learning
...
The automatic phase is owned by `run`. It applies compact invariants under
`.codex/phasemill/rules/` and reusable multi-step procedures under
`.codex/skills/<name>/`; each learned skill has a `SKILL.md` and may include
only the scripts, assets, templates, examples, or references required by that
procedure. A rule may invoke `$<name>` and link
`../../skills/<name>/SKILL.md`. Project learning needs no extra approval, but
support files stay inside the skill directory and cannot introduce new
dependencies or permissions. It validates its own diff, makes at most two
repair attempts, and restores only its own changes when validation still
fails.

`$learn` uses the same contract for an explicitly selected run or PR. Every
user-global change remains unapplied until Phasemill resolves an exact global
destination, shows a fresh combined diff, and receives explicit approval. It
never scans unrelated PR history, guesses a global root, changes plugin
defaults, or commits standalone-run learning implicitly.
